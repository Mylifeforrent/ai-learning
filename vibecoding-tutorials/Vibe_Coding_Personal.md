...
```text
ProjectName/
├── CLAUDE.md             # 根目录，Stage 2 创建初始版，Stage 14 完善
├── docs/
...
- 时间盒：2 小时
- 最小可交付：README + 根目录 CLAUDE.md（初始版）+ docs/00_setup/project_rules.md（一页纸以内也可以，但要素不缺）
...
【步骤 3：创建根目录 CLAUDE.md（初始版）】
在项目根目录创建 CLAUDE.md，写明：
...
4. .claude/settings.json（如采用 Claude Code）
...
- [ ] 根目录 CLAUDE.md 包含：职责边界、.env 与依赖/虚拟环境管理约定（示例：uv）、
...
【任务】在 Stage 2 创建的根目录 CLAUDE.md 初始版基础上，
