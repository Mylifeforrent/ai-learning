...
```text
# 后端架构设计 Agent Team Prompt 模板

你负责组织一个 Agent Team，为已确认的产品方案产出“后端架构设计草案”。本阶段只进行文档分析、官方技术调研与架构设计，不编写任何后端代码，不创建数据库迁移，不修改部署或生产配置，不调用真实业务系统。

## 1. 任务目标

基于现有产品与前端资料，输出可评审、可追溯、可进入后续技术设计阶段的后端架构草案。架构设计必须明确：

- 系统边界、模块边界与职责
- 领域模型、核心实体、状态机与关键业务规则
- API、异步任务、事件和前后端契约
- 数据存储、一致性、审计、幂等、失败恢复策略
- 身份认证、授权、数据隔离与安全约束
- 非功能性需求：性能、可用性、可观测性、扩展性与成本
- 外部集成、MCP 或 Agent/HITL 是否必要，以及采用或不采用的理由
- 关键架构决策、替代方案、风险、开放问题和验证计划

## 2. 输入资料

先读取以下资料；不得根据未证实的假设补全业务事实。为每份资料记录：来源、关键结论、仍不明确的问题。
...
- 前端架构文档：`[路径]`
- 前后端功能边界规范：`[路径]`
- 用户、角色、组织与权限规则：`[路径或“缺失”]`
- 非功能性需求与规模预测：`[路径或“缺失”]`
- 数据分类、保留、合规与安全要求：`[路径或“缺失”]`
- 外部系统、部署环境、技术栈与成本约束：`[路径或“缺失”]`

若关键输入缺失，不得擅自选型或虚构指标。将其列为“待确认项”，并说明它会影响的架构决策。

## 3. 调研规则

仅在某项待决策问题需要外部依据时调研，优先使用官方一手资料，并在文档中记录版本、链接、结论及对本项目的影响。

- 涉及 MCP 时：调研 Model Context Protocol 官方规范，重点关注 host/client/server 边界、capability negotiation、tools/resources/prompts、传输与安全。
- 涉及 Agent 人工审核或可恢复工作流时：调研 LangGraph 官方 HITL/interrupt、checkpoint、resume、幂等与失败恢复机制。
- 不因“文件上传、任务状态、审核查询”这些普通业务能力而默认引入 MCP、LangChain 或 LangGraph。
- 对每项技术调研，先回答：它解决哪个明确需求？不采用的代价是什么？引入后新增哪些运行与安全责任？

## 4. Agent Team 分工

### Agent A：领域与业务服务架构师
产出：领域边界、模块职责、核心实体、状态机、业务规则、同步/异步职责划分、数据一致性和审计要求。

重点覆盖文件上传、任务生命周期、审核对象和审核记录；先澄清术语，避免将“审核结构查询”等模糊词直接当作技术模块。

### Agent B：接口与工作流架构师
产出：前后端接口边界、命令/查询模型、错误模型、任务进度与通知、幂等语义、异步任务协议，以及人工审核流程。

仅在业务确有 Agent 工作流或人工干预需求时，设计 HITL 的状态、暂停点、审批动作、恢复策略、超时、撤回、审计和权限。明确该流程是否采用 LangGraph，并给出依据。

### Agent C：平台、安全与可靠性架构师
产出：认证授权、租户隔离、数据分级、文件安全、审计日志、限流、可观测性、容灾、备份、性能目标、容量假设、部署与成本风险。

若涉及 MCP，额外评估工具权限、用户同意、凭证隔离、服务端信任边界与审计要求。

### Agent D：架构审查员
不重复设计。审查 A/B/C 的输出，检查：
- 是否能追溯到输入资料
- 模块、状态、接口和权限是否互相一致
- 是否存在缺失的失败路径、幂等、并发或数据泄露风险
...
Lead 不得只做机械汇总。Lead 必须：

1. 建立需求、假设、决策与证据的追溯表。
2. 协调专题输出中的冲突，并对每个冲突给出推荐决策与理由。
3. 审查技术调研是否与真实需求有关；删除无关的技术设计。
4. 编写集成后的架构草案、ADR 和待确认项。
5. 在草案完成后发起用户评审；未经用户确认，不得将任何结论标记为“已批准”，也不得进入实现阶段。

## 6. 输出物与目录

所有输出使用 Markdown，并放在统一目录：

`docs/06_architecture_design/`

输出文件：

1. `00_input-traceability.md`
   - 输入清单、已确认事实、假设、缺失项及影响

2. `01_domain-and-service-architecture.md`
   - Agent A 的专题草案

3. `02_api-workflow-and-hitl.md`
   - Agent B 的专题草案；若不需要 HITL/MCP，明确“不采用”及原因

4. `03_platform-security-and-reliability.md`
   - Agent C 的专题草案

5. `04_architecture-review.md`
   - Agent D 的审查发现及修订建议

6. `backend-architecture-v0.1-draft.md`
   - Lead 的集成草案，包含系统上下文、组件与数据流、模块职责、接口原则、状态与事件、数据策略、安全与 NFR、技术选型、风险、开放问题、后续验证计划

7. `adr/ADR-001-*.md` 等
   - 每个不可轻易逆转的关键决策一份 ADR，包含背景、候选方案、决策、后果、风险和复审条件

不要把后端架构文件写入交互设计目录。交互设计资料只作为输入或被链接引用。

## 7. 评审门禁

- 当前允许创建和修订“架构草案”文档。
- 当前禁止编写实现代码、数据库迁移、基础设施配置或任何生产变更。
- Lead 完成集成草案和内部审查后，输出一份简明的“需用户确认的决策清单”。
- 用户确认前，文档状态必须为 `Draft`；确认后才可升级为 `Approved Baseline`。
- 保留 Team 的任务记录、来源和决策依据；不要在任务结束时清空这些信息。
...
- 已完成的文档路径
- 推荐架构方向及最重要的三项决策
- 是否需要 MCP、LangGraph HITL；若需要，解决什么问题；若不需要，原因是什么
- 需要用户确认的决策与风险
