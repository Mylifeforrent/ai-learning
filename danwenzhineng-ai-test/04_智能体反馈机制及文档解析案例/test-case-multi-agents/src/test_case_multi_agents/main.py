...
Flow:
1. A file is read and parsed into confirmed Markdown content.
2. TestCaseWriter generates test cases from the human-confirmed parsed content.
3. TestCaseReviewer reviews the generated test cases.
4. HumanReviewer confirms or requests changes.
5. The latest writer output is saved as the final test cases after approval.
"""
...
""".strip()
    ...
    return f"""
Create high-quality software test cases from the following human-confirmed parsed document content.
...
Human-confirmed parsed content:
{confirmed_content}
""".strip()
...
def create_team(
    ...
    writer = AssistantAgent(
        ...
        system_message=(
            ...
            "Use only the human-confirmed parsed document content as the source of truth. "
            "Analyze the content for business rules, user flows, validation rules, "
    ...
    reviewer = AssistantAgent(
        ...
        system_message=(
            ...
            "latest test cases from TestCaseWriter against the human-confirmed parsed document content. "
...
def create_writer_reviewer_team(
    ...
    writer = AssistantAgent(
        ...
        system_message=(
            ...
            "be clear enough that another tester can execute them without extra context. "
            "Use only the human-confirmed parsed document content as the source of truth. "
            "Analyze it for business rules, table fields, user flows, validation rules, "
            "state transitions, error handling, security/privacy risks, accessibility, "
    ...
    reviewer = AssistantAgent(
        ...
        system_message=(
            ...
            "TestCaseWriter output against the human-confirmed parsed document content "
            "and any human feedback. Assess traceability, coverage, priority, clarity, "
            "observability, duplicate coverage, missing document rules, omitted table "
            "fields, missing exception flows, boundary values, security, accessibility, "
            "performance, and recovery behavior. If ready for human confirmation, start with REVIEW_READY. "
...
def build_revision_task(
    ...
    return f"""
Revise the test cases using the human-confirmed parsed document content, reviewer comments, and human feedback.

Return the complete updated test case table in the required format. Do not only explain changes.

Human-confirmed parsed document content:
{confirmed_content}
...
async def generate_review_cycle_for_web(
    ...
    if human_feedback:
        ...
        task = build_task(confirmed_content=confirmed_content, parsed_document=parsed_document)
