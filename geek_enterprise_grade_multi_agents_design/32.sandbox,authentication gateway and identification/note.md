...
![图片](imgs/01-83cb884c5932da20dced1d247318dbcc.png)
...
![图片](imgs/02-965a532e3baecf805a37d70bcebeaa65.png)
...
![图片](imgs/03-81f582c107b68939d6ce0f99d28690b1.png)
...
![图片](imgs/04-2652f06cf789a5fd8e2490d285a0277a.png)
...
> 💡 课程说明：本节代码已同步至 GitHub，地址：[https://github.com/kid0317/crewai_mas_demo/blob/main/m5l32/](https://github.com/kid0317/crewai_mas_demo/blob/main/m5l32/)

### 3.1 第一层：沙箱——限制工具的执行范围

安全的第一层是物理隔离——工具执行时能看到什么、碰到什么。沙箱做的事就是把 Agent 的可见世界缩到最小。

![图片](imgs/05-25c5f7b5345c096e0d40eba4304bfc5f.png)
...
![图片](imgs/06-c196305yy2bb5c46563667bc261e532c.png)
...
![图片](imgs/07-7faf6ed0c8118a8f59c0fdc637e9acbf.png)
...
![图片](imgs/08-08d7a99fd9efd1cbf8a2e85be19a9bf3.png)
...
1. 沙箱即身份（Egress Identity Injection）。

   Token 写入沙箱环境，出站请求自动注入身份信息。Agent 代码无感知——基础设施层托管认证。
...
1. 四层确定性输入消毒。
   ...
   ：环境变量清洗 / 随机 Nonce / 路径归一化 / 元数据剥离。零 LLM 依赖。

**落地心法：输入消毒用确定性正则规则，不用 LLM 判断——因为 LLM 也可以被绕过。**

1. Defense in Depth——没有单点失败。

   多层防御，每层独立生效：沙箱 → 权限网关 → 身份认证 → 成本围栏 → 可观测性。
