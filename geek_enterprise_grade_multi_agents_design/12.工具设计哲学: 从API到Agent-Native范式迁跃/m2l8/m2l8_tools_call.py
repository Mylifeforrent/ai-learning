...
本示例展示了：
- Hook 机制：如何在工具调用前执行自定义逻辑
- 上下文管理：如何使用上下文变量获取用户信息
- 安全控制：如何防止路径穿越攻击
- 多租户隔离：如何为不同用户创建独立的工作空间

学习要点：
- Hook 装饰器：@before_tool_call 的使用
...
DEMO_ROOT = PROJECT_ROOT.parent / "crewai_mas_demo"

for import_path in (PROJECT_ROOT, DEMO_ROOT):
...
def file_path_hook(context: Any):
    ...
    if path_field:
        ...
        original_file_path = context.tool_input.get(path_field, "")
        ...
        try:
            ...
            # 如果成功，说明路径在工作空间内
            context.tool_input[path_field] = str(original_file_path_abs)
            ...
            try:
                ...
                # 如果成功，使用拼接后的路径
                context.tool_input[path_field] = str(new_file_path_abs)
