...
![图片](imgs/01-b0790475fd1166e564efd2ae0a30c9f6.png)
...
> 💡 课程说明：本节代码已同步至 GitHub，地址：[https://github.com/kid0317/crewai_mas_demo/blob/main/m5l31/](https://github.com/kid0317/crewai_mas_demo/blob/main/m5l31/)

在给出方案之前，先看看 Agent 在生产环境里最常遇到的三个可靠性问题。它们分别指向我们今天要实现的三个策略。

![图片](imgs/02-cbe3c2ccdffaa367b39faf6f43e8c9a0.png)
...
![图片](imgs/03-1490a06cd33d9fc23d6efa99e8d8fyy2.png)
...
![图片](imgs/04-7e94ae98ca239599dd28ebc2e817e8c4.png)
...
> 🧠 **回忆一下**：上节课我们讲了 `dispatch()` 是 fire-and-forget——handler 异常被吞掉，不影响执行。但可靠性策略需要的是**阻断能力**——检测到循环就停、超预算就拒绝。这就是这节要解决的核心工程问题。

### 5.1 dispatch_gate：从报警器到保险丝

![图片](imgs/05-51ea445c3eba80a71d912074a0ca1379.png)
...
- 我们实现了**状态哈希去重的循环检测**——双路径覆盖 ReAct 和原生 FC，配合 `max_iter` 做双保险。哈希去重第 3 次就发现循环，`max_iter` 是最后防线。
- 我们实现了**实时成本围栏**——代码层硬停止，不靠 Agent 自觉。粗估 token 做围栏决策，精确计费走 Langfuse。
