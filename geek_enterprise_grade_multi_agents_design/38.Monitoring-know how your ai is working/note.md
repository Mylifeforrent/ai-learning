...
![图片](imgs/01-d744c64e5eacfcee07228842f8f496ce.png)
...
![图片](imgs/02-2cc73b9e6f6713c25bd38d3afb7c23f8.png)
...
![图片](imgs/03-12d46c2f61a717cd0f5cb2cded90552d.png)
...
![图片](imgs/04-3b4ed43ed105293a04d4c8b394d2613b.png)
...
![图片](imgs/05-70d1476fa39760a3a5555f87768b4a11.png)

| 指标 | 具体内容 |
| --- | --- |
| Token 消耗 | 输入 Token/ 输出 Token**分开统计**（两者单价不同） |
...
![图片](imgs/06-bfcac915e932787a9b9c4769d6yy7ceb.png)
...
![图片](imgs/07-7b8a5254e0ce4d97afcyy65a82ceb4ef.png)
...
![图片](imgs/08-9fd1b632e018b0f8379ccc33aea5929d.png)
...
> 🧠 **回忆一下**：37 课的 CI 第五阶段是安全扫描——事前防御，用 garak 和 promptfoo 在每次变更前跑。第六层是**持续运行的第二道防线**：覆盖那些只在真实生产流量里才会出现的攻击向量，CI 测试集无法预测。

**这层监控的不是"指标趋势"，而是"检测合规事件"。**

![图片](imgs/09-89a8145a844c556c4e478436cf43b3e7.png)
...
![图片](imgs/10-b68dfdeffdb57047230e4ff0896eddfb.png)
...
![图片](imgs/11-fd8fb06cb2fe6c2ee681b04ef8af3a32.png)
...
- 我们掌握了**六层监控体系**：系统层（必须有但不够）→ 成本层（最易忽视的定时炸弹）→ Agent 执行层（传统监控的完全盲区）→ 质量层（最难量化，最重要）→ 业务层（最终判断标准）→ 安全合规层（检测事件不是监控趋势）。大多数团队只到第一层，真正的失败几乎都发生在第三层以上。
